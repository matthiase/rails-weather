class WeatherController < ApplicationController
  def index
    ...
    @current_conditions = @weather[:current]
    ...
    @localtime = Time.at(@current_conditions[:dt]).in_time_zone(@weather[:timezone])
    @sunrise = Time.at(@current_conditions[:sunrise]).in_time_zone(@weather[:timezone])
    @sunset = Time.at(@current_conditions[:sunset]).in_time_zone(@weather[:timezone])
  end
end
